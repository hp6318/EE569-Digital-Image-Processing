...
// the following code for read and write image file is taken from the source code provided
{
    ...
    unsigned char Imagedata_yuv[Size_x][Size_y][BytesPerPixel];
    ...
    for (int i=0;i<Size_x;i++){
        for (int j=0;j<Size_y;j++){
            contrast_a[i][j][0]=floor(cdf[(int)Imagedata_yuv[i][j][0]]*255);
            contrast_a[i][j][1]=Imagedata_yuv[i][j][1];
            contrast_a[i][j][2]=Imagedata_yuv[i][j][2];
        }
    }
    
    //convert RGB to YUV color space using the given conversion in appendix
    unsigned char Imagedata_rgb[Size_x][Size_y][BytesPerPixel];
    for (int i=0;i<Size_x;i++){
        for (int j=0;j<Size_y;j++){
            Imagedata_rgb[i][j][0]=1.164*(contrast_a[i][j][0] - 16) + 1.596*(contrast_a[i][j][2] - 128);
            Imagedata_rgb[i][j][1]=1.164*(contrast_a[i][j][0] - 16) - 0.813*(contrast_a[i][j][2] - 128)- 0.391*(contrast_a[i][j][1] - 128);            
            Imagedata_rgb[i][j][2]=1.164*(contrast_a[i][j][0] - 16) + 2.018*(contrast_a[i][j][1] - 128);
        }
    }
    cout<<"Hello"<<endl;
	...
	fwrite(Imagedata_rgb, sizeof(unsigned char), Size_x*Size_y*3, file);
	fclose(file);
