...
unsigned char Imagedata[512][768][3];
unsigned char img_padd[516][772][3];
...
// the following code for read and write image file is taken from the source code provided
{
    ...
    for (int i=0;i<padd_size_x-4;i++){
        for (int j=0;j<padd_size_y-4;j++){
            ...
            img_padd[i+2][j+2][1]=Imagedata[i][j][1];
            img_padd[i+2][j+2][2]=Imagedata[i][j][2];
        }
    }
    //cout<<5<<endl;
    ...
    for (int i=0;i<padd_size_x;i++){
        ...
        img_padd[i+1][1][1]=Imagedata[i-1][1][1];
        img_padd[i+1][1][2]=Imagedata[i-1][1][2];
    }
    //cout<<'a'<<endl;
    ...
    for (int i=0;i<padd_size_x;i++){
        ...
        img_padd[i+1][0][1]=Imagedata[i-1][2][1];
        img_padd[i+1][0][2]=Imagedata[i-1][2][2];
    }
    //cout<<'b'<<endl;
    ...
    for (int i=0;i<padd_size_x;i++){
        ...
        img_padd[i+1][padd_size_y-2][1]=Imagedata[i-1][padd_size_y-6][1];
        img_padd[i+1][padd_size_y-1][1]=Imagedata[i-1][padd_size_y-7][1];

        img_padd[i+1][padd_size_y-2][2]=Imagedata[i-1][padd_size_y-6][2];
        img_padd[i+1][padd_size_y-1][2]=Imagedata[i-1][padd_size_y-7][2];
    }
    //cout<<'c'<<endl;
    ...
    for (int i=0;i<padd_size_y;i++){
        ...
        img_padd[1][i][1]=img_padd[3][i][1];
        img_padd[0][i][1]=img_padd[4][i][1];

        img_padd[1][i][2]=img_padd[3][i][2];
        img_padd[0][i][2]=img_padd[4][i][2];
    }
    //cout<<'d'<<endl;
    ...
    for (int i=0;i<padd_size_y;i++){
        ...
        img_padd[padd_size_x-2][i][1]=img_padd[padd_size_x-4][i][1];
        img_padd[padd_size_x-1][i][1]=img_padd[padd_size_x-5][i][1];

        img_padd[padd_size_x-2][i][2]=img_padd[padd_size_x-4][i][2];
        img_padd[padd_size_x-1][i][2]=img_padd[padd_size_x-5][i][2];
    }
    //cout<<6<<endl;
    //code for convoultion with linear filter
	unsigned char img_denoised [Size_x][Size_y][3];
	//cout<<7<<endl;
    srand(time(0));
	for (int i=2;i<Size_x+2;i++){
		for (int j=2;j<Size_y+2;j++){
            ...
            img_denoised[i-2][j-2][0]=img_padd[i+row_idx][j+col_idx][0];
            img_denoised[i-2][j-2][1]=img_padd[i+row_idx][j+col_idx][1];
            img_denoised[i-2][j-2][2]=img_padd[i+row_idx][j+col_idx][2];
        }
    }
	...
	fwrite(img_denoised, sizeof(unsigned char), Size_x*Size_y*3, file);
	fclose(file);
	return 0;
}
